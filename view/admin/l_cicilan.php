...
<page>
        ...
        $qupdate = "SELECT * FROM cicilan GROUP BY peminjaman";
        $rupdate = mysqli_query($mysqli, $qupdate);
        while($dupdate = mysqli_fetch_assoc($rupdate)){
		$peminjaman=$dupdate['peminjaman'];	
       		?>
			<tbody>
<tr>
<td align="center">&nbsp;<?php echo $no ?>&nbsp;</td>
<td style="width:100px;" align="center">&nbsp;Rp. <?php echo  number_format($dupdate['peminjaman'],0); ?>&nbsp;</td>
 <?php
       
   
        $q = "SELECT * FROM cicilan  WHERE peminjaman = '$peminjaman'";
        $a = mysqli_query($mysqli, $q);
        while($b = mysqli_fetch_assoc($a)){
		$perbulan = number_format($b['perbulan'],0,".",".");
		echo "<td  style='width:60px;' >Rp. $perbulan</td>";
		}		
       		?>
