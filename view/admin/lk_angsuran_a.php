...
<page>
		...
		<hr>
		 <?php
		 $no=1;
		 $id_angsuran_a = $_GET["id_angsuran_a"];
				$mysqli= mysqli_connect("localhost","u8110790_us_kop","gUcZ1){kMrsl","u8110790_db_koperasi");
   
        $qupdate = "SELECT * FROM angsuran_a, anggota, admin WHERE angsuran_a.id_anggota = anggota.id_anggota AND angsuran_a.username = admin.username AND angsuran_a.id_angsuran_a ='$id_angsuran_a' ";
        $rupdate = mysqli_query($mysqli, $qupdate);
        while($dupdate = mysqli_fetch_assoc($rupdate)){
			
       		?>
		
		<h5 align="center">BUKTI TRANSAKSI ANGSURAN KREDIT ALAT PERTANIAN</h5>
		 <table border="0"   style="font-size:12px;" cellspacing="3" cellpadding="3">
       <tr>
        <td width="40" align="rigth"></td>
        <td width="410" colspan="4">Pada Tanggal  <?php echo $dupdate['tgl_angsuran_a']; ?> :</td>  
     ...
     <tr>
      ...
      <tr>
        <td width="40" align="rigth"></td>
        <td width="410" colspan="4">Telah Melakukan Transaksi Angsuran Kredit Yang ke <?php echo $dupdate['angsuran_ke']; ?> Yaitu :</td>  
      </tr>
